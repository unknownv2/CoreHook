...
{
    public sealed partial class ProcessManager2 : IProcessManager2
    {
        public Process ProcessHandle { get; private set; }
        public SafeProcessHandle SafeHandle { get; private set; }

        private readonly IMemoryManager _memoryManager;
        public ProcessManager2(IMemoryManager memoryManager)
        {
            _memoryManager = memoryManager;
        }

        public ProcessManager2(Process process)
        {
        ...
        private SafeProcessHandle GetReadProcessHandle(int processId)
        {
        ...
        private IntPtr ExecuteFuntion(string module, string function, byte[] arguments, bool canWait = true)
        {
                ...
                Interop.Advapi32.ProcessOptions.PROCESS_VM_WRITE))
            {
                ...
                IntPtr remoteAllocAddr = Interop.Kernel32.VirtualAllocEx(
                    ...
                    Interop.MemoryAllocationType.Commit | Interop.MemoryAllocationType.Reserve,
                    Interop.MemoryProtection.ReadWrite);
                ...
                try
                {
                    if (!Interop.Kernel32.WriteProcessMemory(
                        ...
                        arguments,
                        arguments.Length,
                        out IntPtr bytesWritten) || bytesWritten.ToInt32() != arguments.Length)
                    {
                        throw new Win32Exception("Failed to allocate memory in remote process.");
                    }
